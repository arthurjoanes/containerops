...
const images=path.join(root,'docs/screenshots/interface-v2');
const evidence=path.join(root,'docs/evidence/interface-v2');
...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#recovery .operation-summary strong').first().innerText(),'8');
  assert.equal(await page.locator('.recovery-job').getAttribute('open'),'');
  ...
  assert.equal(await page.locator('.scan-findings.clear').count(),1);
  assert.equal(await page.locator('.scan-findings').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(255, 255, 255)');
  await page.goto(report+'#recovery');
  await page.emulateMedia({reducedMotion:'no-preference'});
  assert.ok((await page.locator('.operation-link').first().evaluate(e=>getComputedStyle(e).transitionDuration)).includes('0.18s'));
  ...
  assert.equal(await page.locator('#operations .section-details .job').isVisible(),true);
  ...
  for(const [name,panel,expected] of [
   ...
   ['backup-only','recovery','Nenhum resultado registrado'],
  ...
  await plain.locator('#rollback .section-details > summary').click();
  assert.equal(await plain.locator('#rollback .section-details .job').isVisible(),true);
